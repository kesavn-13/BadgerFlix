  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        // Load all courses in parallel (much faster!)
        ...
        // Build the coursesBySubject object
  ...
  return (
    <div className="min-h-screen bg-netflix-black">
      ...
      {continueWatching.length > 0 && (
        <div className="px-8 -mt-32 relative z-10 pb-8">
          <div className="mb-6">
            <h2 className="text-3xl font-bold text-white flex items-center gap-3">
              <span className="w-1 h-8 bg-netflix-red rounded-full"></span>
              Continue Watching
            </h2>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
            {continueWatching.map((item) => (
              ...
              >
                <div className="relative aspect-video rounded-lg overflow-hidden transition-all duration-300 group-hover:scale-105">
                  <div className="absolute inset-0 bg-gradient-to-br from-netflix-red/40 via-blue-600/40 to-indigo-600/40 flex items-center justify-center">
      ...
      {/* Subject Rows */}
      <div className={`px-8 ${continueWatching.length > 0 ? 'pb-16' : '-mt-32 relative z-10 pb-16'}`}>
        {subjects.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-gray-400 text-xl">No courses available. Upload a lecture to get started!</p>
          </div>
        ) : (
          subjects.map((subject) => (
            <div key={subject} className="mb-16 animate-fade-in">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-gray-400 text-sm">
                  {coursesBySubject[subject]?.length || 0} {coursesBySubject[subject]?.length === 1 ? 'Course' : 'Courses'}
                </span>
              </div>
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide" style={{ scrollBehavior: 'smooth' }}>
              {coursesBySubject[subject]?.map((course, idx) => {
                // Generate a gradient based on course index for variety
                const gradients = [
                  "from-purple-600/40 via-blue-600/40 to-indigo-600/40",
                  "from-red-600/40 via-orange-600/40 to-pink-600/40",
                  "from-green-600/40 via-teal-600/40 to-cyan-600/40",
                  "from-yellow-600/40 via-amber-600/40 to-orange-600/40",
                  "from-blue-600/40 via-purple-600/40 to-pink-600/40",
                ];
                ...
                return (
                  <div key={course.id} className="flex-shrink-0 w-64 group animate-fade-in relative">
                    ...
                    >
                      <div className="relative aspect-video rounded-lg overflow-hidden transition-all duration-300 group-hover:scale-105 group-hover:shadow-2xl group-hover:shadow-netflix-red/50 group-hover:z-10">
                        {/* Background with gradient - Movie Poster Style */}
                        <div className={`absolute inset-0 bg-gradient-to-br ${gradient} flex items-center justify-center`}>
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <div className="bg-netflix-red rounded-full p-4 transform scale-0 group-hover:scale-100 transition-transform">
